...
## flowing ribbon for every river, and (optionally) a clipped plane for every inland
## lake. River courses come from tools/build_rivers.py.
##
## How it fits the project
## -----------------------
## The terrain streams, so the whole heightfield is never in memory at once. The
## OFFLINE tool tools/build_rivers.py traces the rivers painted into the Azgaar biome
## map (the inland blue lines) into centreline polylines, samples a ground height along
## each, and writes:
##     terrain/water_bodies.json      sea level + per-river { points: [x,y,z,width] }
##                                    (+ optional per-lake { level, bbox, mask })
## This node just renders that result. Nothing here recomputes anything, and no
## ArrayMesh is built at runtime per chunk (the terrain notes warn against per-chunk
## runtime mesh churn): the sea is one PlaneMesh and each river is one ribbon ArrayMesh,
## all created ONCE at startup (bounded work).
##
## Ocean
## -----
...
## Setup
## -----
##   1. Run `python3 tools/build_rivers.py` once (re-run after re-exporting the terrain,
##      ideally with --chunks pointing at the .glb folder so river heights are exact).
...
## Side length (m) of the player-following sea plane. Make it reach the camera far plane
## (this scene's camera far is 16 km) so open ocean has no visible plane edge.
@export var ocean_size: float = 16000.0
## Plane subdivisions per axis (vertices for the wave displacement). 320 -> 50 m grid on
## a 16 km plane (~257k verts). Higher = smoother swell, more verts; the fine ripple
## detail is per-fragment (analytic normal) so this only needs to carry the big swell.
@export var ocean_subdiv: int = 320
...
@export var river_view_distance: float = 2200.0
